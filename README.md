...
##Adding custom choice fields
If you would like to add custom choice fields to your choices, set these in `config.js`. An example, `Reputation`, has been added to the config for illustration purposes. `type` and `value` fields must be included in all entries. If no field type is given, it will default to text. All html input types (`text`, `number`, etc) are honored.
